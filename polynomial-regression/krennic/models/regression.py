...
class PolynomialRegressionModel(RegressionModel):
    ...
    def build(self, x: np.ndarray, y: np.ndarray):
        ...
        cached_coefficients_of_design_matrix = np.array(
            [np.sum(x**p) for p in range(self.__degree * 2 + 1)]
        )
        design_matrix = np.array(
            [
                [
                    cached_coefficients_of_design_matrix[i + j]
                    for j in range(self.__degree + 1)
                ]
                for i in range(self.__degree + 1)
            ]
        )
        response_vector = np.array(
            [np.sum(y * (x**i)) for i in range(self.__degree + 1)]
        )
    ...
    def fit(self, x: float | np.ndarray) -> float | np.ndarray:
        ...
        return np.sum(
            [
                self.__polynomial_coefficients[i] * (x**i)
                for i in range(self.__degree + 1)
            ],
            axis=0,
        )
