...
def apply_regression_model(
    df: pd.DataFrame, x_column: str, y_column: str, model: RegressionModel
) -> pd.DataFrame:
...
def train_polynomial_regression_model(
    df: pd.DataFrame, x_column: str, y_column: str, degree: int
) -> RegressionModel:
    return __train_polynomial_regression_model(
        x=df[x_column], y=df[y_column], degree=degree
    )
...
def train_ridge_regression_model(
    df: pd.DataFrame, x_column: str, y_column: str, degree: int, λ: float
) -> RegressionModel:
    return __train_ridge_regression_model(
        x=df[x_column], y=df[y_column], degree=degree, λ=λ
    )
...
def train_lasso_regression_model(
    ...
    return __train_lasso_regression_model(
        x=df[x_column], y=df[y_column], degree=degree, λ=λ
    )
